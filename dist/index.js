...
// src/coverageTypes.ts
var Convert = class {
  static toCoverageReport(json) {
    return cast(JSON.parse(json), r("CoverageReport"));
  }
  static coverageReportToJson(value) {
...
// src/coverageMarkdown.ts
var import_fs = require("fs");
var generateUncoveredLines = (lineMap) => {
  let sortedKeys = Object.keys(lineMap).sort((a2, b) => parseInt(a2) > parseInt(b) ? 1 : -1);
  let results = [];
  let currentRun = -1;
  let runStart = -1;
...
var pReport = (r2, threshold) => {
  return `${percent(r2)} ${emoji(r2, threshold)}`;
};
var percent = (r2) => {
  return `${(r2.covered / r2.total * 100).toFixed(2)}`;
};
var emoji = (r2, threshold) => {
  const percent2 = r2.covered / r2.total * 100;
  if (percent2 < threshold) {
    return "\u{1F534}";
  }
  const remainder = 100 - threshold;
  const band = remainder * 0.1;
  if (percent2 < threshold + band) {
    return "\u{1F7E1}";
  }
  return "\u{1F7E2}";
};
var generateCoverageMarkdown = (reportPath, threshold) => {
  const json = (0, import_fs.readFileSync)(reportPath);
  ...
  for (let k in files) {
    const o2 = files[k];
    allResult.statements.total += o2.statements.total;
    allResult.statements.covered += o2.statements.covered;
    allResult.branch.total += o2.branch.total;
    allResult.branch.covered += o2.branch.covered;
    allResult.functions.total += o2.functions.total;
    allResult.functions.covered += o2.functions.covered;
    allResult.lines.total += o2.statements.total;
    allResult.lines.covered += o2.statements.covered;
  }
...
|All files| ${pReport(allResult.statements, threshold)} | ${pReport(
  ...
  for (let k in files) {
    ...
    markdown += `
| - ${file}| ${pReport(r2.statements, threshold)} | ${pReport(r2.branch, threshold)} |${pReport(
      r2.functions,
      threshold
    )} |${pReport(r2.statements, threshold)} | ${r2.uncovered} |`;
  }
  return markdown;
};

// src/app.ts
var CovereageReporter = class {
  constructor() {
    this.path = core2.getInput("path", { required: true });
    this.threshold = core2.getInput("threshold", { required: true });
    this.token = core2.getInput("token", { required: true });
    this.context = getCheckRunContext();
    this.octokit = github2.getOctokit(this.token);
  }
  async run() {
    let threshold = parseInt(this.threshold, 10);
    if (isNaN(threshold)) {
      core2.setFailed(`Input parameter 'threshold' has invalid value`);
      return;
    }
    if (!(0, import_fs2.existsSync)(this.path)) {
      core2.setFailed(`Could not find report file at: ${this.path}`);
      return;
    }
    try {
      ...
      core2.setFailed(`Could not generate report`);
      throw error;
    }
  }
};
async function main() {
  try {
    ...
    if (error instanceof Error) {
      ...
      core2.setFailed(JSON.stringify(error));
    }
  }
}
main();
